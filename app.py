...
def gerar_os(funcionario, df_pgr, riscos_selecionados, epis_manuais, medicoes_manuais, riscos_manuais, modelo_doc_carregado):
    ...
    for med in medicoes_manuais:
        ...
        epi_associado = str(med.get('epi_name', med.get('epi', ''))).strip()  # CORREÇÃO: verifica múltiplos campos possíveis
        ...
        if agente and agente not in ['', 'N/A', 'nan', 'None'] and valor and valor not in ['', 'N/A', 'nan', 'None']:
            ...
            if unidade and unidade not in ['', 'N/A', 'nan', 'None']:
                linha += f" {unidade}"
            if epi_associado and epi_associado not in ['', 'N/A', 'nan', 'None']:  # CORREÇÃO: usar epi_associado
...
def main():
    ...
    with st.container(border=True):
        ...
        # Consolidar todos os riscos selecionados
        riscos_selecionados = []
        for categoria_key, categoria_display in CATEGORIAS_RISCO.items():
            if categoria_key in riscos_selecionados_pgr and riscos_selecionados_pgr[categoria_key]:
                riscos_selecionados.extend(riscos_selecionados_pgr[categoria_key])
