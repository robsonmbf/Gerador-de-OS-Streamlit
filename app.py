        ...
        for paragrafo in doc.paragraphs:
            for placeholder, valor in substituicoes.items():
                if placeholder in paragrafo.text:
                    # Processar runs para preservar a formatação
                    for run in paragrafo.runs:
                        if placeholder in run.text:
                            parts = run.text.split(placeholder)
                            run.text = parts[0]  # Mantém a parte antes do placeholder com a formatação original
                            new_run = paragrafo.add_run(str(valor)) # Adiciona o valor sem negrito
                            new_run.bold = False # Garante que o valor inserido não seja negrito
                            paragrafo.add_run(parts[1]) # Adiciona a parte depois do placeholder com a formatação original
                            break # Sai do loop de runs, pois o placeholder foi tratado
                ...
                for celula in linha.cells:
                    for placeholder, valor in substituicoes.items():
                        if placeholder in celula.text:
                            # Processar runs para preservar a formatação
                            for run in celula.paragraphs[0].runs: # Assumindo que o placeholder está no primeiro parágrafo da célula
                                if placeholder in run.text:
                                    parts = run.text.split(placeholder)
                                    run.text = parts[0]  # Mantém a parte antes do placeholder com a formatação original
                                    new_run = celula.paragraphs[0].add_run(str(valor)) # Adiciona o valor sem negrito
                                    new_run.bold = False # Garante que o valor inserido não seja negrito
                                    celula.paragraphs[0].add_run(parts[1]) # Adiciona a parte depois do placeholder com a formatação original
                                    break # Sai do loop de runs, pois o placeholder foi tratado
                    ...
                    with st.expander(f"{categoria_nome} ({qtd_opcoes} opções)", expanded=False):
                        ...
                        if st.session_state.agentes_risco[categoria_key]:
                            ...
                            for idx, risco in enumerate(st.session_state.agentes_risco[categoria_key]):
                                col_risco, col_edit, col_remove = st.columns([4, 1, 1])
                                with col_risco:
                                    ...
                                    st.write(risco_text)
                                with col_edit:
                                    if st.button("Editar", key=f"edit_{categoria_key}_{idx}"):
                                        st.session_state[f\"edit_risco_{categoria_key}_{idx}\"] = True
                                        st.rerun()
                                with col_remove:
                                ...
                                if st.session_state.get(f\"edit_risco_{categoria_key}_{idx}\", False):
                                    st.markdown(f\"<div class=\"info-msg\">Editando: {risco[\"agente\"]}</div>\", unsafe_allow_html=True)
                                    col_edit_int, col_edit_unid, col_edit_save = st.columns([2, 2, 1])
                                    with col_edit_int:
                                        edited_intensidade = st.text_input(
                                            "Intensidade:",
                                            value=risco.get(\"intensidade\", \"\"),
                                            key=f\"edited_intensidade_{categoria_key}_{idx}\"
                                        )
                                    with col_edit_unid:
                                        edited_unidade = st.selectbox(
                                            "Unidade:",
                                            UNIDADES_DE_MEDIDA,
                                            index=UNIDADES_DE_MEDIDA.index(risco.get(\"unidade\", \"Não aplicável\")),
                                            key=f\"edited_unidade_{categoria_key}_{idx}\"
                                        )
                                    with col_edit_save:
                                        st.markdown(\"<br>\", unsafe_allow_html=True) # Espaçamento para alinhar o botão
                                        if st.button("Salvar", key=f"save_edit_{categoria_key}_{idx}"):
                                            st.session_state.agentes_risco[categoria_key][idx][\"intensidade\"] = edited_intensidade
                                            st.session_state.agentes_risco[categoria_key][idx][\"unidade\"] = edited_unidade
                                            st.session_state[f\"edit_risco_{categoria_key}_{idx}\"] = False
                                            st.rerun()
